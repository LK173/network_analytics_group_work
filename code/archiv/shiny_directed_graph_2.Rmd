...
output$graph_summary <- renderText({
  ...
  output$degree_distribution <- renderPlot({
    ...
    hist(degree.subgraph,
         main = "Degree Distribution of the original g.directed",
         xlab = "Degree", ylab = "Number of clubs", col = "red", border = "white")
  })

  # Render the new measures
  output$graph_measures <- renderText({
    ...
    paste("Statistical summary of the original graph:",
          "\nAvg. degree of the directed graph:", avg.degree,
          "\nClustering coefficient of the directed graph:", clustering.coeff,
          "\nAvg. path length of the directed graph:", avg.path.length)
  })
}
```
...
```{r}
# Test built in filter Page directed graph
# Shiny app UI
ui <- fluidPage(
  titlePanel("Overview about the football Transfers Network"),
...
server <- function(input, output) {
  ...
  output$centrality_table <- renderDataTable({
    ...
    # Filter player data based on the selected club or the built-in filter of the interactive graph
    if (!is.null(selected.club())) {
      df.centrality <- df.centrality %>% filter(club_name == selected.club())
    } else if (!is.null(graph_selected_club()) && length(graph_selected_club()) > 0) {
      df.centrality <- df.centrality %>% filter(club_name %in% graph_selected_club())
    }
...
```{r}
# Backup
# Shiny app UI Built in function
ui <- fluidPage(
  titlePanel("Overview about the football Transfers Network"),
