...
ui <- navbarPage(
  ...
  navbarMenu(title = "Network Exploration",
             ...
             tabPanel(title = "Subpage 3.1",
                      fluidRow(
                        ...
                        column(10,
                               wellPanel(
                                 h3("Summary Statistics"),
                                 plotlyOutput("map", height = "600px", width = "100%")
                               )
                        )
                      )
             ),
             ...
             # 3.2 Subpage
             tabPanel(title = "Subpage 3.2 Details about Football 
                      Transfers Network",
                      fluidPage(
                            ...
                            fluidRow(
                              column(4,
                                     ...
                                     selectInput("season_3_2", 
                                                 ...
                                                 selected = 2021, 
                                                 width = "100%"),
                                     selectInput("country_3_2", "Country:", 
                                                 choices = NULL),
                                     selectInput("position_3_2", "Position:", 
                                                 choices = c("All")),
                                     tags$br(),
                                     tags$br(),
                          ...
                          fluidRow(
                            column(4,
                                   ...
                                   selectInput("season_4_2", "Select a Season:",
                                               choices = 2004:2021, 
                                               selected = 2021, width = "100%"),
                                   selectInput("country_4_2", "Country:", 
                                               choices = NULL),
                                   selectInput("club_4_2", "Club:", 
                                               choices = NULL),
                                   selectInput("position_4_2", "Position:", 
                                               choices = c("All")),
                                   tags$br(),
                                   tags$br(),
                                   h4("Explanations for the Usage of the 
...
server <- function(input, output, session) {
  ...
  output$map <- renderPlotly({
    ...
    p <- p + geom_point(aes(x = dealing_country_lon, y = dealing_country_lat, label = dealing_country), data = season.data_3_1(), color = "lightblue", size = 1)
    
    # Add points for each node in the dataset
    p <- p + geom_point(aes(x = country_lon, y = country_lat, label = country), data = season.data_3_1(), color = "red", size = 1)
  ...
  season.data_3_2 <- reactive({
    data %>%
      ...
      filter(if (input$position_3_2 != "All") 
        position_category == input$position_3_2 else TRUE)
  })
  ...
  observe({
    ...
    updateSelectInput(session, "country_3_2", "Country:", 
                      choices = country.choices_3_2())
  })
  ...
  observe({
    ...
    position.filtered_3_2 <- data %>%
      ...
      distinct(position_category) %>%
      pull(position_category)
    updateSelectInput(session, "position_3_2", "Position:", 
                      choices = c("All", position.filtered_3_2))
  })
  ...
  output$degree_distribution <- renderPlot({
    ...
    ggplot() +
      ...
      labs(x = "Degree", y = "Number of clubs",
           title = paste("Degree Distribution of ", input$country_3_2)) +
      theme_minimal() +
      theme(plot.margin = margin(15, 15, 15, 15),
            plot.background = element_rect(fill = "#F5F5F5"),
            panel.background = element_rect(fill = "#F5F5F5"),
  ...
  season.data_4_2 <- reactive({
    data %>%
      ...
      filter(if (input$position_4_2 != "All") position_category == 
               input$position_4_2 else TRUE)
  })
  ...
  observe({
    updateSelectInput(session, "country_4_2", "Country:", 
                      choices = country.choices_4_2())
  })
  
  observe({
    req(input$country_4_2)
    clubs.filtered_4_2 <- data %>% filter(season == input$season_4_2, 
                                          country == input$country_4_2) %>% 
      distinct(club) %>% pull(club)
    updateSelectInput(session, "club_4_2", "Club:", 
                      choices = clubs.filtered_4_2)
  })
  
  observe({
    req(input$club_4_2)
    position.filtered_4_2 <- position.choices_4_2()
    updateSelectInput(session, "position_4_2", "Position:", 
                      choices = c("All", position.filtered_4_2))
  })
  
  # Plot directed graph interactive of top performers---------------------------
  df.performers <- reactive({
    season.filtered_4_2 <- season.data_4_2() %>% 
      filter(country == input$country_4_2, club == input$club_4_2)
    
    
    # Filter for top performers based on age and performance score, 
    # and keep only top 20------------------------------------------------------
    performers.top <- 
      season.filtered_4_2[order(-season.filtered_4_2$normalized_performance),
                          ][1:10, ]
  ...
  filtered.data_4_2 <- reactive({
    season.filtered_4_2 <- 
      season.data_4_2() %>% filter(country == input$country_4_2, club ==
                                     input$club_4_2)
  ...
  output$prediction_graph <- renderVisNetwork({
    ...
    g.test <- 
      graph_from_data_frame(df.top.5.pred.dealing.clubs[, c("dealing_club", 
                                                            "club", 
                                                            "transfer_prob")],
                                    directed = TRUE)
    ...
    V(g.test)$color <- ifelse(V(g.test)$name == input$club_4_3, "red", "blue")
