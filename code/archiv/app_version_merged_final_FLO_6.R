                          ...
                          fluidRow(
                            column(6,
                                   ...
                                   sliderInput("season.statistics", 
                                               "Season", min = min(df.transfers.network$season),
                                               max = max(df.transfers.network$season), 
                                               value = c(min(df.transfers.network$season), 
                                                         max(df.transfers.network$season)), 
                                               step = 1, sep =""),
                                   selectInput("summary.f1", "Countries", 
                                               choices = c("Select All", 
                                                           unique(df.transfers.network$country))
                                               )
                            ),
                            column(6,
...
server <- function(input, output, session) {
  ...
  output$country.ui.3.1 <- renderUI({
    df.country.filtered.3.1 <- df.transfers.network %>% filter(season == input$season.3.1) %>%
  ...
  output$map <- renderPlotly({
    ...
    p <- p + theme_void() + theme(legend.position = "bottom")
  ...
  observe({
    ...
    clubs.filtered.4.2 <- df.transfers.network %>% filter(season == input$season.4.2, 
                                          country == input$country.4.2) %>% 
