...
ui <- navbarPage(
  ...
  navbarMenu(title = "Network Analysis",
                          ...
                          fluidRow(
                            column(4,
                                   ...
                                   selectInput("position_4_1", "Select position:",
                                               c("All", "Goalkeeper", "Defender", "Midfielder", "Attack"))
                            ),
                            column(8,
                                   style = "border-left: 1px solid #999; padding-left: 10px;",
             ...
             tabPanel(title = "4.3 Football Transfers Predictions (Based on Season 2017-2021)",
                      fluidRow(
                        column(12,
                               h2("Football Transfers Predictions (Based on Season 2017-2021)"),
                               p("In this section, we will analyze the football transfers data and provide predictions based on the selected criteria."),
                               br()
                        )
                      ),
                      wellPanel(
                        wellPanel(
                          fluidRow(
                            column(4,
                                   h3("Choose a country, club, position, and number of players to show"),
                                   selectInput("country", "Country:", unique(data$country)),
                                   uiOutput("club_ui_4_3"),
                                   selectInput("position", "Position:", c("All", "Goalkeeper", "Defense", "Midfield", "Attack")),
...
server <- function(input, output) {
  ...
  top.market.value.countries <- reactive({
    if (input$summary.f1 == "Select All") {
      ...
      geom_bar(stat = "identity", fill = "#0066cc", color = "white") +
      labs(x = "Country",
           y = "Average Market Value (in Millions)") +
      theme_minimal() +
      theme(plot.margin = margin(15, 15, 15, 15),
            plot.background = element_rect(fill = "#F5F5F5"),
            panel.background = element_rect(fill = "#F5F5F5"),
  ...
  output$country_ui_3_2 <- renderUI({
    countries.filtered <- data %>% filter(season == input$season_3_2) %>% distinct(country) %>% pull(country)
    selectInput("country", "Country:", choices = countries.filtered)
  })
  ...
  output$position_ui_3_2 <- renderUI({
    ...
    selectInput("position", "Position:", choices = c("All", position.filtered))
  })
  ...
  output$position_ui_4_2 <- renderUI({
    ...
    position.choices <- data %>%
      filter(season == input$season_4_2, country == input$country, club == input$club) %>%
      distinct(position_category) %>%
      pull(position_category)
    selectInput("position", "Position:", choices = c("All", position.choices))
  })
  
  # Update the country input based on the selected season
  output$country_ui_4_2 <- renderUI({
    countries.filtered <- data %>% filter(season == input$season_4_2) %>% distinct(country) %>% pull(country)
    selectInput("country", "Country:", choices = countries.filtered)
  })
  
  output$club_ui_4_2 <- renderUI({
    req(input$country) # Ensure input$country is available before proceeding
    clubs.filtered <- data %>% filter(season == input$season_4_2, country == input$country) %>% distinct(club) %>% pull(club)
    selectInput("club", "Club:", choices = clubs.filtered)
  })
