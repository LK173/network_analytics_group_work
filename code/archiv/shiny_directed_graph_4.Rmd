...
df.football.transfer <- df.football.transfer %>% filter(season >= 2002 &
                                                          season <= 2021)
...
server <- function(input, output) {
  ...
  position.choices <- reactive({
    data %>%
      filter(season == input$season, country == input$country, club == input$club) %>%
  ...
  output$country_ui <- renderUI({
    countries.filtered <- data %>% filter(season == input$season) %>% distinct(country) %>% pull(country)
  ...
  output$club_ui <- renderUI({
    ...
    clubs.filtered <- data %>% filter(season == input$season, country == input$country) %>% distinct(club) %>% pull(club)
...
server <- function(input, output) {
  ...
  output$position_ui <- renderUI({
    req(input$country) # Ensure input$country is available before proceeding
    position.choices <- data %>%
      filter(season == input$season, country == input$country) %>%
      distinct(position_category) %>%
      pull(position_category)
    selectInput("position", "Position:", choices = c("All", position.choices))
  })
