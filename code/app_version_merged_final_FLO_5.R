...
server <- function(input, output, session) {
  ...
  output$country.ui.3.1 <- renderUI({
    df.country.filtered.3.1 <- data %>% filter(season == input$season.3.1) %>%
  ...
  observe({
    ...
    clubs.filtered.4.2 <- data %>% filter(season == input$season.4.2, 
  ...
  output$prediction.graph <- renderVisNetwork({
    ...
    V(g.top.clubs)$color <- 
      ifelse(V(g.top.clubs)$name == input$club_4_3, "red", "blue")
