...
g.directed <- graph_from_data_frame(df.football.transfer[, c("source", "target", "name")], directed = TRUE)
...
summary(g.directed)
#3514 unique delaing_clubs and clubs
# we have here 3514 nodes --> no too much nodes
```
...
g.directed <- graph_from_data_frame(
  df.football.transfer[, c("dealing_club", "club", "name")], directed = TRUE)

# Plot the directed graph without labels, small nodes, and edges
plot(g.directed, vertex.size = 5, edge.arrow.size = 0.5, vertex.label.cex = 0,
     edge.color = "gray", vertex.label = NA, vertex.color = "lightblue")

# Set vertex colors --> dealing_club red and club blue
