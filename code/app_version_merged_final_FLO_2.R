...
server <- function(input, output, session) {
  ...
  output$map <- renderPlotly({
    ...
    p <- p + geom_segment(aes(x = dealing_country_lon, y = dealing_country_lat, xend = country_lon, yend = country_lat, color = weight), data = edge_counts, arrow = arrow(length = unit(0.2, "cm"))) +
      scale_color_gradient(low = "yellow", high = "red") # Set the color gradient
    
    p <- p + geom_point(aes(x = dealing_country_lon, y = dealing_country_lat, label = dealing_country), data = season.data_3_1(), color = "green", size = 2)
