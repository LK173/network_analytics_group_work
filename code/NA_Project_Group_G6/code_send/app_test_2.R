...
# File:         App
# 
#               This file contains both the user interface and server-side 
#               code for the app.
#
# ------------------------------------------------------------------------------
...
server <- function(input, output, session) {
  ...
  output$map <- renderPlotly({
    ...
    p <- p + geom_point(aes(x = dealing_country_lon, 
                        ...
                        data = season.data.3.1(), color = "lightblue", size = 2)
  ...
  observe({
    ...
    selected.club(unique(clubs_filtered)[1]) # Set the initial selected.club value
  })
  
  # Update selected.club when input$club.4.3 changes
  observe({
    req(input$club.4.3)
    selected.club(input$club.4.3)
  })
  ...
  observeEvent(input$prediction_graph_selected, {
    selected.club(input$prediction_graph_selected)
  })
